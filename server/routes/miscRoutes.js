module.exports = (auth, db, priceService, solanaGrpcService) => { 
  ...
  router.get('/tokens/age/:mint', auth.authRequired, async (req, res) => {
    try {
      const { mint } = req.params;
      
      if (!mint || typeof mint !== 'string' || mint.length < 32 || mint.length > 44) {
        console.log(`[${new Date().toISOString()}] ❌ Invalid mint length: ${mint?.length || 'null'}`);
        return res.status(400).json({
          success: false,
          error: 'Invalid mint address format',
          details: 'Mint address must be a valid Solana address (32-44 characters)'
        });
      }

      const solanaAddressRegex = /^[1-9A-HJ-NP-Za-km-z]+$/;
      if (!solanaAddressRegex.test(mint)) {
        console.log(`[${new Date().toISOString()}] ❌ Invalid mint characters: ${mint.slice(0, 8)}...`);
        return res.status(400).json({
          success: false,
          error: 'Invalid mint address characters',
          details: 'Mint address contains invalid characters for Solana address'
        });
      }
      ...
      try {
        tokenData = await Promise.race([
          priceService.getTokenData(mint),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Token data timeout')), 8000))
        ]);
      } catch (error) {
        console.warn(`[${new Date().toISOString()}] ⚠️ Price service timeout for ${mint.slice(0, 8)}...`);
      }

      let ageData = null;

      if (tokenData && tokenData.age) {
        const ageInHours = tokenData.age.ageInHours || null;
        const createdAt = tokenData.age.createdAt || null;
        
        ageData = {
          createdAt,
          ageInHours,
          isNew: (ageInHours || 999) < 24,
          source: 'price_service'
        };
      } else {
        console.warn(`[${new Date().toISOString()}] ⚠️ No age data available for ${mint.slice(0, 8)}...`);
        ageData = {
          createdAt: null,
          ageInHours: null,
          isNew: false,
      ...
      res.json({
        ...
        meta: {
          mint: mint.slice(0, 8) + '...',
          processingTime: duration,
          dataSource: ageData.source,
          cached: !!tokenData?.lastUpdated
        }
      });

    } catch (error) {
      console.error(`[${new Date().toISOString()}] ❌ Error in token age endpoint:`, error.message);
      res.status(500).json({
        success: false,
        error: 'Failed to fetch token age',
